...
const AddCourseInformation = ({ isOpen, onClose, getInfo }) => {
  ...
  const getRoles = async () => {
    try {
      const { data } = await axiosInstance.get(endPoints.roles.getAll);
      setRoles(data.data);
    } catch (error) {
      toast.error(error);
    }
  ...
  const handleAddCourseList = async () => {
    ...
    if (
      ...
      try {
        ...
        toast.success("Add CourseList Successfully.");
        getInfo();
        onClose();
        setLoader(false);
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[9999] overflow-y-auto">
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg w-full max-w-[90%] max-h-[90%] sm:w-[450px] lg:w-[400px] overflow-y-auto">
        <h2 className="text-xl font-bold mb-4">Add Course Information</h2>
              ...
              >
                ...
                {courseList.map((course) => (
                  <option value={course._id}>
                    {course.courseType.length > 45
                      ? course.courseType.slice(0, 45) + "..."
                      : course.courseType}
                  </option>
                ))}
              </select>
